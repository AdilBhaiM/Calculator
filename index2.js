...
equalButton.addEventListener('click', () => {
    if (currNum || userInput[userInput.length - 1] === ")" || userInput[userInput.length - 1] === "(") {
        if (currNum) {
...
numbers.forEach(num => {
    num.addEventListener('click', () => {
        if (allOperators.includes(userInput[userInput.length - 1]) && userInput[userInput.length - 1] !== ')')
